...
For real shared use, all users must open the same deployed public link.

You will only get this real link after deploying the project on Render, Railway, or another Node.js hosting service. It will look similar to a Render/Railway website URL, but the exact link is generated by the hosting platform.

```text
Use the real link given by Render/Railway after deployment.
```
...
```text
Admin -> the real deployed AttendPro link
Employee -> the same real deployed AttendPro link
Phone QR -> the same real deployed AttendPro link
```
